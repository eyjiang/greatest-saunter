...
/* ---------------- photos ----------------
   one key per photo, never listed or scanned — the id in the URL is the only
   way in. Upstash allows 100MB per value and 10MB per request, so a
   client-downscaled JPEG is nowhere near either limit. */

const photoKey = (id) => `saunter:photo:${id}`;

export async function writePhoto(id, contentType, buffer) {
  if (useRedis) {
    await redis('SET', photoKey(id), JSON.stringify({ contentType, b64: buffer.toString('base64') }));
    return `/api/photo/${id}`;
  }
  const blob = await put(`photos/${id}`, buffer, { access: 'public', contentType });
  return blob.url;
}

export async function readPhoto(id) {
  if (!useRedis) return null; // blob-hosted photos are served straight from their own URL
  const d = parse(await redis('GET', photoKey(id)));
  if (!d || !d.b64) return null;
  return { contentType: d.contentType, buffer: Buffer.from(d.b64, 'base64') };
}
